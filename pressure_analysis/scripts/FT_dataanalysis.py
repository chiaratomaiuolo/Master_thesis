...
from scipy.signal import butter, freqs, lfilter, sosfilt
...
if __name__ == "__main__":
    ...
    ax1.set_ylabel(r'$P_{4}$ [mbar]', color='red')
    ax1.plot(t_hours, (P4-expo(t_hours,*popt))/(np.max(P4)-np.min(P4)),\
            color='red', label=r'$P_{4}$ [mbar]')
    ...
    ax2.plot(t_hours, T_butterworth/((max(T_butterworth)-min(T_butterworth))),\
            label=fr'T_{{eff}} Butterworth filtered , $f_{{cutoff}} = {f_cutoff:.2e}$ [Hz]')
